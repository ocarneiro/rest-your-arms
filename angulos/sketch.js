...
function draw() {
  ...
  fill(255, 255, 255);
  textSize(32);
  text("ombro:" + Math.trunc(ombroDireito.x) + "," + Math.trunc(ombroDireito.y), 25, tamanho.y-90);
  text("mão:" + Math.trunc(maoDireita.x) + "," + Math.trunc(maoDireita.y), 25, tamanho.y-55);
  cateto_oposto = Math.trunc(ombroDireito.y) - Math.trunc(maoDireita.y);
  cateto_adjacente = Math.trunc(ombroDireito.x) - Math.trunc(maoDireita.x);
  atan_calculado = Math.atan2(cateto_oposto, cateto_adjacente);
  text("atan(" + cateto_oposto + "," + cateto_adjacente + ") = " +
               atan_calculado.toFixed(2) + " rad", 25, tamanho.y-25);
  angulo = Math.floor(atan_calculado * 180 /Math.PI);
  textSize(64);
  text(angulo + "º",tamanho.x - 160, 75);
}
